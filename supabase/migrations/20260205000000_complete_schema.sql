...
-- Create profiles table before functions that reference it
...
-- Lovely people table for the public showcase page
CREATE TABLE public.lovely_people (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  "Name" TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

ALTER TABLE public.lovely_people ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone can view lovely people"
  ON public.lovely_people FOR SELECT
  USING (true);

CREATE POLICY "Anyone can insert lovely people"
  ON public.lovely_people FOR INSERT
  WITH CHECK (true);

CREATE POLICY "Admins can update lovely people"
  ON public.lovely_people FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can delete lovely people"
  ON public.lovely_people FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role));
